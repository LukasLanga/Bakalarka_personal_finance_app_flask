...
def invite_user_route(account_id):
    ...
    if not invited_email or not role:
        return jsonify({"detail": "Invalid data: invited_email and role are required."}), 400
    ...
    try:
        ...
        error_message = str(e)
        if "409_conflict" in error_message:
            error_key = error_message.replace("409_conflict:", "").strip()
            return jsonify({"detail": error_key}), 409
...
def get_account_users_route(account_id):
    ...
    if not user_role:
        return jsonify({"detail": "Forbidden: You do not have access to this account."}), 403
